...
<div class="text-[#f17f18] hover:text-[#f19718] my-5 cursor-pointer ease-in-out duration-300 {$locale == "en" ? "text-Bahij_Plain" : "text-Bahij_Plain_Plain"}"  on:click={toggleShowFullText}>
  {showFullText ? $_("showLess") : $_("showMore") }
</div>
</div>

<div class="w-full flex justify-center items-center text-center mt-12">
  <p class="text-black dark:text-white font-bold py-3 mt-3 md:text-4xl {$locale == "en" ? "text-Bahij_Plain" : "text-Bahij_Plain_Plain"}" data-aos="zoom-out-up" data-aos-anchor-placement="center" data-aos-duration="1000">
