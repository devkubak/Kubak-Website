  ...
  onMount(async () => {
    try {
      technologies = await technologyStore.getAll() as TechnologyDto[];
       ("Technologies", $technologyStore.data);
    } catch (e) {
      console.log(e);
    } finally {
      isLoading = false;
      ...
      <div class="w-full h-[540px] flex flex-col justify-end items-center " >
        ...
        <div class="w-full h-12 flex justify-between items-center text-center bg-[#ffffff9a] dark:bg-[#21212192]" >
          ...
          {#if technologies.length > 0}
            <p class="w-full h-12 text-center flex justify-center items-center bg-white dark:bg-[#3a3a3a] dark:text-white text-2xl font-bold " style="font-family:Bahij;">{technologies[currentTechnologyIndex].title[$locale + ""]}</p>
          {/if}
            ...
            {#key currentTechnologyIndex}
              {#each technologies[currentTechnologyIndex].technologyAttributes as attr, index}
                ...
                id="card-shadow"
                  ...
                  <p class="w-full max-h-12 text-12 text-center flex justify-center items-center py-2 bg-[#f1f1f1] dark:bg-[#191919] dark:text-white rounded-b-lg"  style="font-family: Bahij_Plain;">
                    {attr.title}
                  </p>
                </div>
              {/each}
